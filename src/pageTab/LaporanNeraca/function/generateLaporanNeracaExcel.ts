...
import { fetchGenerateLaporanNeraca } from './fetchGenerateLaporanNeraca';

export async function exportGenerateLaporanNeracaExcel(
  companyId: string, 
  token: string, 
  startDate: string, 
  endDate: string, 
  ...
  try {
    // Fetch fresh data from generate endpoint
    const freshData = await fetchGenerateLaporanNeraca(companyId, token, startDate, endDate);

    // Split data into assets (1xx) and liabilities/equity (2xx and 3xx)
    const leftData = freshData.filter((row: any) => 
      row.kode_akun?.startsWith('1') || (row.kode_akun === null && row.nama_akun?.includes('Aset'))
    );
    const rightData = freshData.filter((row: any) => 
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      if (left) {
        ...
        const namaAkun = indent + (left.nama_akun || '');
        const nilaiCell = left.nilai !== null && left.nilai !== undefined ? left.nilai.toLocaleString('id-ID') : '';
        
        worksheet.getCell(`A${rowIdx}`).value = left.kode_akun || '';
        worksheet.getCell(`B${rowIdx}`).value = namaAkun;
        worksheet.getCell(`C${rowIdx}`).value = nilaiCell;
      ...
      if (right) {
        ...
        const namaAkun = indent + (right.nama_akun || '');
        const nilaiCell = right.nilai !== null && right.nilai !== undefined ? right.nilai.toLocaleString('id-ID') : '';
        
        worksheet.getCell(`E${rowIdx}`).value = right.kode_akun || '';
        worksheet.getCell(`F${rowIdx}`).value = namaAkun;
        worksheet.getCell(`G${rowIdx}`).value = nilaiCell;
