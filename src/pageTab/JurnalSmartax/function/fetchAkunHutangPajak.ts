...
export const fetchAkunHutangPajak = async (
  id: string,
  companyId: string,
  token: string,
  filter?: FilterInput
) => {
  return await fetchAkunHutangPajakBackend(id, companyId, token, filter);
};

const fetchAkunHutangPajakBackend = async (
  id: string,
  companyId: string,
  token: string,
  filter?: FilterInput
  ...
  try {
    const response = await axios.get(`http://127.0.0.1:5000/akun-hutang-objek-pajak/${companyId}/${id}`, {
      headers: {
