...
export const fetchAkunPerkiraanSub = async (data: AkunPerkiraan, token: string) => {
  return await fetchAkunPerkiraanBackend(data, token);
};

const fetchAkunPerkiraanBackend = async (data: AkunPerkiraan, token: string) => {
  try {
    const response = await axios.get(`http://127.0.0.1:5000/akun-perkiraan/sub/${data.companyId}`, {
      headers: {
